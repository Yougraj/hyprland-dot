...
generate_cache() {
    local wallpaper="$1"
    # local hash=$(basename "$wallpaper" | md5sum | cut -d ' ' -f1)
    local hash="wall"
...
# Set the picked wallpaper using swww (without using the generated cache files)
swww img "$random_wallpaper" --transition-fps 60 --transition-type any --transition-duration 3
